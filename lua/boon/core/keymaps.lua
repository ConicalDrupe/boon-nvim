...
vim.keymap.set(
	{ "n", "i" },
	"<leader>ic",
	"<esc>i<cr>```{python}<cr>```<esc>0",
	{ desc = "[i]nsert python [c]ode chunk" }
)
...
vim.keymap.set(
	"n",
	"<leader>cR",
	":split term://R --no-save<cr> <bar> <C-w><C-p>",
...
vim.keymap.set(
	{ "n" },
	"rip",
	'"ryiw vip :sno/<C-R>r//g<left><left>',
	{ desc = "[R]eplace word [I]nside [P]aragraph " }
)
...
vim.keymap.set(
	{ "v" },
	"rip",
	'"ry vip :sno/<C-R>r//g<left><left>',
	{ desc = "Visual [R]eplace word [I]nside [P]aragraph " }
)
